...
# Fetch API instead of DB connection to avoid missing dependencies locally
try:
    resp = requests.get(f"{API}/timetable-results", timeout=5)
    if resp.status_code != 200:
        print(f"Erreur HTTP: {resp.status_code}")
        sys.exit(1)
    
    results_list = resp.json()
    target_result = next((r for r in results_list if str(r['id']) == str(DB_ID)), None)
    
    if not target_result:
        print(f"Erreur: Aucun résultat trouvé avec l'id {DB_ID}.")
        sys.exit(1)
        
    timetable = target_result.get('data', [])
    db_name = target_result.get('name') or f"Export_{target_result.get('algo_type')}"
...
for entry in timetable:
    ...
    target_sids = set()
    if str(entry.get('section_id')) != 'None': target_sids.add(str(entry.get('section_id')))
    ...
    for sid in target_sids:
        ...
        section_data[str(sid)].setdefault(slot_key, []).append(rec)
...
def apply_header(ws, title):
    ...
    c = ws.cell(1, 1, f"FSTG Timetabling - {title.upper()}")
    c.font=Font(bold=True, size=18, color="FFFFFF"); c.fill=PatternFill("solid","1F2F3F")
...
ws_g.sheet_view.showGridLines = False; apply_header(ws_g, f"Planning Salles - {db_name}")
...
for sid in sorted(section_data.keys(), key=lambda x: sections_dict.get(x, '')):
    ...
    ws.sheet_view.showGridLines = False; apply_header(ws, f"Emploi du Temps - {name}")
    col_map = {0:2, 1:3, 2:5, 3:7, 4:8}
    for i,l in enumerate(FSTG_LABELS):
        c=ws.cell(3,col_map[i],l); c.font=Font(bold=True); c.alignment=Alignment(horizontal='center'); c.fill=PatternFill("solid","FAD7A0"); set_border(ws,3,3,col_map[i],col_map[i])
    for di, dn in enumerate(ALL_DAYS):
        ...
        dj=ws.cell(rs,1,dn); dj.font=Font(bold=True,color="FFFFFF"); dj.fill=PatternFill("solid","2C3E50"); dj.alignment=Alignment(horizontal='center',vertical='center',text_rotation=90); set_border(ws,rs,rs+SUB_ROWS-1,1,1)
        for i in range(5):
            col=col_map[i]; assigns=section_data[str(sid)].get((dn,FSTG_LABELS[i]), [])
            if not assigns:
                ws.merge_cells(start_row=rs, start_column=col, end_row=rs + SUB_ROWS - 1, end_column=col)
                ws.cell(rs, col).fill = PatternFill("solid", "F2F3F4"); set_border(ws, rs, rs + SUB_ROWS - 1, col, col)
            else:
        ...
        for sc in [4, 6]:
            ...
            ws.cell(rs, sc).fill = PatternFill("solid", "ECF0F1"); set_border(ws, rs, rs + SUB_ROWS - 1, sc, sc)
