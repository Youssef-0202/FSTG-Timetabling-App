...
target_section_id = sys.argv[2] if len(sys.argv) > 2 else ""
...
OUTPUT_PATH = os.path.join(BASE_DIR, "algorithms", "ga_sa_hybrid", "v2", "logs", f"FSTG_EXCEL_PREMIUM{file_suffix.upper()}{'_' + target_section_id if target_section_id else ''}.xlsx")
...
if not target_section_id:
    ...
    ws_g.sheet_view.showGridLines = False; apply_header(ws_g, "Planning Global des Salles")
...
for sid in sorted(section_data.keys(), key=lambda x: sections_dict.get(x, '')):
    if target_section_id and str(sid) != str(target_section_id):
        continue
...
wb.save(OUTPUT_PATH)
print("DONE: EXCEL PREMIUM FINAL - BORDERS EVERYWHERE")
