...
class HybridEngine:
    ...
    def evolve(self):
        ...
        while len(new_gen) < self.pop_size:
            ...
            # On n applique le SA que si l enfant est PROMETTEUR (mieux que son meilleur parent)
            best_parent = p1 if p1.fitness < p2.fitness else p2
            score_child = self.get_score(child)
            
            if score_child < best_parent.fitness:
                # L'enfant est meilleur : on le polit avec le SA
                child = self.simulated_annealing_search(child, iterations=self.sa_iterations // 4)
                new_gen.append(child)
                sa_impact_list.append(max(0, score_child - child.fitness))
            else:
                # L'enfant est moins bon : on garde le parent intact (Zéro SA = Gain de temps)
                new_gen.append(copy.deepcopy(best_parent))
