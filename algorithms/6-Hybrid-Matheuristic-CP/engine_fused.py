...
from solver_cp import LocalCPSolver
...
class HybridEngine:
    ...
    def apply_exact_repair(self):
        """
        Applique une réparation mathématique (CP) sur le meilleur individu.
        """
        best = self.population[0]
        solver = LocalCPSolver(self.dm, self)
        
        print(f" [CP-SOLVER] Tentative de réparation exacte (Score actuel: {best.fitness:.1f})...")
        
        # Sauvegarde pour rollback en cas d'échec
        old_fitness = best.fitness
        self._old_h = best.h_violations
        self._old_soft = best.soft_penalty
        self._old_details = best.soft_penalty_detail
        old_assignments = [(a.room, a.timeslot) for a in best.assignments]
        
        # Tentative 1 : Stabilité des salles
        changed = solver.refine_room_stability(best)
        
        # Tentative 2 : Réduction des Gaps (Compactage)
        changed_gaps = solver.compact_sections(best)
        changed = changed or changed_gaps
        
        if changed:
            best.fitness = None
            new_score = self.get_score(best)
            
            if new_score < old_fitness:
                print(f" [CP-SOLVER] Réparation réussie. Nouveau score: {new_score:.1f} (Gain: {old_fitness - new_score:.1f})")
            else:
                # ROLLBACK COMPLET : Restaurer les salles, les créneaux ET les métriques
                for i, (r, t) in enumerate(old_assignments):
                    best.assignments[i].room = r
                    best.assignments[i].timeslot = t
                
                # Restaurer les attributs de score
                best.fitness = old_fitness
                best.h_violations = getattr(self, '_old_h', 0)
                best.soft_penalty = getattr(self, '_old_soft', 0)
                best.soft_penalty_detail = getattr(self, '_old_details', {})
                
                print(f" [CP-SOLVER] Rollback : La réparation dégrade le score ({new_score:.1f} > {old_fitness:.1f}).")
        else:
            print(" [CP-SOLVER] Aucune amélioration mathématique immédiate trouvée.")
