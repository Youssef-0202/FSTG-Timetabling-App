...
def run_grid_search():
    ...
    csv_file = os.path.join(os.path.dirname(__file__), "grid_search_rl_results.csv")
    
    with open(csv_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['pop_size', 'mutation_rate', 'sa_iterations', 'sa_cooling', 'time_sec', 'hard_score', 'soft_score'])

    print(f"=== DEMARRAGE GRID SEARCH RL CONTROLLER ({len(combinations)} combinaisons) ===")
    
    best_overall_score = float('inf')
    best_overall_params = None

    for idx, params in enumerate(combinations):
        print(f"\n[{idx+1}/{len(combinations)}] Test RL avec: {params}")
    ...
    print(f"\n" + "="*50)
    print(" FIN DU GRID SEARCH RL CONTROLLER")
