...
def run_fused_optimization():
    ...
    for gen in range(1, MAX_GEN + 1):
        ...
        # --- ACTIVATION DU CP-SOLVER (MATHEURISTIQUE) ---
        # Si on stagne depuis 20 gén, ou toutes les 40 gén
        if (no_improvement_count >= 20) or (gen % 40 == 0):
            engine.apply_exact_repair()
            # Mettre à jour le best après réparation
            if engine.population[0].fitness < best_overall_score:
                best_overall_score = engine.population[0].fitness
                no_improvement_count = 0
