...
class HybridEngine:
    ...
    def evolve(self):
        """
        Execute une generation complete avec Optimisations TURBO :
        - Tri par index (évite les copies d'objets lourds)
        - Elitisme à 30%
        - Remplacement par Parent si meilleur (Competitive Children)
        - SA systématique après crossover/mutation
        """
        # 1. Tri par index pour la vitesse (P4 optimization)
        pop_indices = sorted(range(len(self.population)), key=lambda i: self.get_score(self.population[i]))
        
        # 2. Elitisme 30% (Comme demandé)
        n_elite = max(1, int(self.pop_size * 0.3))
        new_gen = [copy.deepcopy(self.population[pop_indices[i]]) for i in range(n_elite)]
        
        sa_impact_list = [] 

        # 2.5 SA Intensif sur l'Elite (800 iterations par defaut en turbo)
        for i in range(len(new_gen)):
            old_fit = new_gen[i].fitness
        ...
        # 3. Reproduction et Competition Parent-Enfant
        while len(new_gen) < self.pop_size:
            # Selection par Tournoi
            idx1 = random.choice(pop_indices[:max(2, self.pop_size//2)])
            idx2 = random.choice(pop_indices[:max(2, self.pop_size//2)])
            ...
            # --- LOGIQUE TURBO : COMPETITIVE CHILDREN ---
            # Si le meilleur parent est meilleur que l'enfant brut, on garde le parent
            best_parent = p1 if p1.fitness < p2.fitness else p2
            score_child = self.get_score(child)
            
            final_individual = child
            if best_parent.fitness < score_child:
                final_individual = copy.deepcopy(best_parent)
                final_individual.fitness = best_parent.fitness
            
            # d. SA Light pour chaque nouvel individu (25% des iterations)
            old_fit_ind = final_individual.fitness
            final_individual = self.simulated_annealing_search(final_individual, iterations=self.sa_iterations // 4)
            sa_impact_list.append(max(0, old_fit_ind - final_individual.fitness))

            new_gen.append(final_individual)

        # -- ANALYSE DIVERSITE --
        avg_sa_impact = sum(sa_impact_list) / len(sa_impact_list) if sa_impact_list else 0
