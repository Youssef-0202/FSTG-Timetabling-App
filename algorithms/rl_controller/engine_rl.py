...
class HybridEngine:
    ...
    def __init__(self, data_manager, 
        ...
        self.constraints_mask = constraints_mask or {
            ...
            "S_LUNCH": True, "S_STABILITY": True, "S_FREE_APM": True
        }
        
        # ── RL Agent ──
        self.agent = agent or QLearningAgent(actions=[0, 1, 2, 3])
    ...
    def simulated_annealing_search(self, schedule):
        ...
        for it in range(self.sa_iterations):
            ...
            if active_phase == 'hard':
                ...
                chosen_action = -1 # On ne tracke pas la phase hard pour le RL soft (optionnel)
            else:
                # RL CHOICE
                chosen_action = self.agent.choose_action(current_state)
                saved_indices, saved_states = self._apply_soft_move_rl(schedule, unlocked, chosen_action)
    ...
    def _apply_soft_move_rl(self, schedule, unlocked, action):
        """Mouvements Metier dirigés par l'agent RL."""
        changed_indices, old_states = [], []

        if action == 0: # STABILISATION DES SALLES
        ...
        elif action == 1: # COMPACTAGE PROFESSEUR
            idx = random.choice(unlocked)
            a = schedule.assignments[idx]
            prof_id = a.module_part.teacher_id
            prof_assigns = [schedule.assignments[i] for i in unlocked if schedule.assignments[i].module_part.teacher_id == prof_id and i != idx]
            if prof_assigns:
        ...
        elif action == 2: # COMPACTAGE ÉTUDIANT
            idx = random.choice(unlocked)
            a = schedule.assignments[idx]
            sec_id = a.module_part.section_id
            sec_assigns = [schedule.assignments[i] for i in unlocked if schedule.assignments[i].module_part.section_id == sec_id and i != idx] if sec_id else []
            if sec_assigns:
        ...
        else: # EXPLORATION ALÉATOIRE (ACTION 3)
            changed_indices, old_states = self._apply_hard_move(schedule, unlocked)
