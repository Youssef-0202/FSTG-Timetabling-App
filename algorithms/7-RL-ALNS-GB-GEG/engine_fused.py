...
def op_random_shift(schedule, unlocked, dm):
    ...
    if not valid_slots: valid_slots = dm.timeslots
...
def op_swap_slots(schedule, unlocked, dm):
    ...
    # Vérification Sanctuarisation avant SWAP
    if a.module_part.unavailable_slots and other.timeslot.id in a.module_part.unavailable_slots:
        return op_random_shift(schedule, unlocked, dm)
    if other.module_part.unavailable_slots and a.timeslot.id in other.module_part.unavailable_slots:
        return op_random_shift(schedule, unlocked, dm)
...
def op_kempe_chain(schedule, unlocked, dm):
    ...
    # Blindage Sanctuarisation : On vérifie que slot_b est safe pour aa et slot_a safe pour bb
    for i in aa:
        if schedule.assignments[i].module_part.unavailable_slots and slot_b.id in schedule.assignments[i].module_part.unavailable_slots:
            return op_random_shift(schedule, unlocked, dm)
    for i in bb:
        if schedule.assignments[i].module_part.unavailable_slots and slot_a.id in schedule.assignments[i].module_part.unavailable_slots:
            return op_random_shift(schedule, unlocked, dm)
...
class HybridEngine:
    ...
    def _build_greedy_individual(self):
        ...
        # Suivi spécifique pour GB-GEG S2 blocks & parallelism
        group_block_occupancy = {}   # (gid, day, block_id) -> count
        section_slot_occupancy = {} # (sid, slot_id) -> count
        ...
        partner_logic = {1:2, 2:1, 3:4, 4:3, 5:6, 6:5}

        # Tri : GB-GEG d'abord, puis par taille de groupe
        def sort_priority(mp):
            score = 0
            if mp.section_id == gb_geg_s2_id: score += 1000
            score += mp.group_size
            return score
        ...
        for mp in mp_shuffled:
            ...
            if not v_slots: v_slots = self.dm.timeslots

            # Augmentation massive pour garantir H=0
            candidate_slots = random.sample(v_slots, min(60, len(v_slots)))
            ...
            for slot in candidate_slots:
                ...
                # Test de TOUTES les salles (plus sûr)
                for room in self.dm.rooms:
                    if room.capacity < mp.group_size: continue
                    cost = 0
                    # Pénalités alignées sur M=1,000,000
                    ...
                    if is_gb_geg:
                        if is_lunch: cost += 8000 # BANNIR LUNCH !
                        if bid:
                            # Parallelism: Bonus si d'autres groupes sont déjà dans ce créneau
                            sec_occ = section_slot_occupancy.get((gb_geg_s2_id, slot.id), 0)
                            if 0 < sec_occ < 4: cost -= 100 # GROS BONUS pour parallele
                            elif sec_occ >= 4: cost += 50
                            
                            for gid in mp.td_group_ids:
                                g_num = get_gr_num(gid)
                                count = group_block_occupancy.get((gid, slot.day, bid), 0)
                                if count == 1: cost -= 200 # Completer le bloc de 4h
                                elif count == 0: cost += 20 
                                if g_num in partner_logic:
                                    p_num = partner_logic[g_num]
                                    # Optimisation : On ne cherche le P_GID que si nécessaire
                                    p_count = sum(1 for k, v in group_block_occupancy.items() if k[1] == slot.day and k[2] == bid and f"Gr {p_num}" in self.dm.group_map.get(k[0], ""))
                                    if p_count > 0: cost -= 100 # Partner Sync
                    
                    if cost < best_cost:
                        best_cost, best_room, best_slot = cost, room, slot
                        if cost <= -300: break

                if best_cost <= -300: break
            ...
            if is_gb_geg:
                section_slot_occupancy[(gb_geg_s2_id, best_slot.id)] = section_slot_occupancy.get((gb_geg_s2_id, best_slot.id), 0) + len(mp.td_group_ids)

            for gid in mp.td_group_ids: 
                group_slot_used[(gid, best_slot.id)] = True
                if is_gb_geg and bid:
                    key = (gid, best_slot.day, bid)
                    group_block_occupancy[key] = group_block_occupancy.get(key, 0) + 1
