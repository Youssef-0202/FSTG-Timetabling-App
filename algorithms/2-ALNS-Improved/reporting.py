...
def initialize_log_file(params, db_stats):
    ...
    header = [
        ...
        f" - Algorithme          : {params.get('ALGORITHM', 'N/A')}",
        f" - Population Size      : {params.get('POP_SIZE', 'N/A')}",
        f" - Max Generations      : {params.get('MAX_GEN', 'N/A')}",
        f" - Mutation Rate        : {params.get('MUTATION_RATE', 'N/A')}",
        f" - SA Iterations        : {params.get('SA_ITERATIONS', 'N/A')}",
        f" - SA Temp Initial      : {params.get('SA_TEMP', 'N/A')}",
        f" - SA Cooling           : {params.get('SA_COOLING', 'N/A')}",
        "\n2. STATISTIQUES DE LA BASE DE DONNEES :",
        f" - Nombre d'Enseignants : {db_stats.get('nb_teachers', 'N/A')}",
        f" - Nombre de Salles     : {db_stats.get('nb_rooms', 'N/A')}",
        f" - Nombre de Sections   : {db_stats.get('nb_sections', 'N/A')}",
        f" - Seances a placer     : {db_stats.get('nb_module_parts', 'N/A')}",
        f" - Creneaux disponibles : {db_stats.get('nb_slots', 'N/A')}",
        "\n" + "=" * 60,
